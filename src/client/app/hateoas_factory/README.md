...
* Setting up a new hateoas focused service
* Overview of the main functions and properties
* Working with paged data 
* Working with single items
* Custom Query Parameters
* Using an item collection with the Grid module
* Events
...
## Main Functions and Important Properties
The service definition is used to extend HATEOASFactory which grants us a handful of handy powers.

### get(options)
Used to obtain paged info. Returns a promise. Options object can have a number of possible properties:
...
### setItem(item)
Extends a services item with 'Link Functions'. If an item is passed into the function it is attached to service.item; otherwise the attached item is used.

### linkName(options) "Link functions"
Set of functions built from the hal envelope. Returns a promise. Ex: meter_reads -> Service.item.meterReads(), hello-world -> Service.item.helloWorld()

### all(options)
Wait for all of an attached items _links to be called before executing.

### getPage(pageNumber, resultSize)
Convience function for page navigation.
...
Navigating pages returned by a link function. Note: this only works for the item currently attached to the service. 

Using reporting as an example:

```js
    Reports.get().then(function() {
        // at some point we do
        Reports.setItem(report);
    });

    // Now we want to use Reports.item.self()
    // Can also access: Reports.item.params and Reports.item.data 
    // Reports.item.columnDefs and/or Reports.item.columns must be defined
    // Can pass typical options object to self()
    Reports.item.self().then(function() {
        // results in Reports.item.data
        Grid.display(Reports.item);
    });

    // Example of getting the next page
    Reports.item.next().then(function() {
        // results in Reports.item.data
    });

    // Using params
    Reports.item.self({page:1, size:100}).then(function() {
        // results in Reports.item.data
    });
...
Now that we have something in Contacts.item a set of new functions related to the _links outline become available. For example; lets assume the items hal envelope had a self, and an account _link defined:

```js
    Contacts.item.self().then(function(fullServerResponse) {
        // data in Contacts.item.data.self;
        // embedded name used in parsing the response taken from Contacts
    });

    // Sometimes we will need to define the emedded array key
    Reports.item.madc({
        emeddedName: 'reportData'
    }).then(function(fullServerResponse) {
        // data in Reports.item.data.madc;
    });

    Contacts.item.account().then(function(fullServerResponse) {
...
```js
    ...
    Contacts.all({
        ...
        // Data in Contacts.item.data.linkName
    });

```

```js
    Contacts.get('/123-XYT?hello=world').then(function(processedResponse) {
        // this makes a call to contacts/123-XYT?hello=world based on no HAL data
