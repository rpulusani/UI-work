...
```js
    {
        ...
        // a hal item
        item: {},
        // seek out a linkName in options.item/self.item
        linkName: '',
        // override url for this call
        url: '',
        meth0d: '' // change the request method
    }
```
...
#### refresh(serviceDefinition)
Updates a services base information with the information in an envelope. If no options are passed in the service resets its values to those it had when first initialized.

#### all(options)
Wait for all of an attached self.items._links to be called before executing. All() has a few options not found in the other functions:
...
Want to navigate paged data returned by a link function? **You can't!** Create a new service with HATEOASFactory and inject it into scope; and do something like the following:

```js
    MeterReads.refresh(Devices.item).then(function() {
        Grid.display(MeterReads, $scope);
    });
```

### Getting single items
The services item{} property is set sometime after an initial paging call (as we do not know the end point to call).
...
```js
    Contacts.get({
        link: 'self', // looking at Contacts.item._links.self
    ...
    Contacts.get({
        link: 'account' // looking at Contacts.item._links.self
    }).then(function(account) {
        Account.item = account;

        Account.get().then(function() {
            Grid.display(Account, $scope);
        });
    });

```

Service.all() lets you fire all links before execution, remember that it will wait for all calls to resolve before continuing; so in some cases you may prefer to call the links asynchronously.

```js
    Contacts.all().then(function(res) {
        // Data is in Contacts.allData.linkName
...
Making a GET request to a defined string against the services endpoint:

```js
    Contacts.get('/123-XYT?hello=world').then(function(processedResponse) {
        // this makes a GET call to contacts/123-XYT?hello=world based on no HAL data
...
```js
    ...
    Devices.get({link: 'meter-reads'}).then(function(meterReads) {
...
```js
    ...
    // Also possible to call initial self link

    Contact.self(options).then(function() {
